...
from    mymodels.layernorm import LayerNorm2d
# helpers
...
class DRSA(nn.Module):
    def __init__(self, channel_num=32, bias = True, ffn_bias=True, window_size=3, with_pe=False, dropout=0.0):     ##window_size=8
        super(DRSA, self).__init__()
        w = window_size

        self.B_SA = nn.Sequential(
            Rearrange('b d (x w1) (y w2) -> b x y w1 w2 d', w1 = w, w2 = w),  # block-like attention
            PreNormResidual(channel_num, Attention(dim = channel_num, dim_head = channel_num//2, dropout = dropout, window_size = window_size, with_pe=with_pe)),
            Rearrange('b x y w1 w2 d -> b d (x w1) (y w2)'),
            # Conv_PreNormResidual(channel_num, Gated_Conv_FeedForward(dim = channel_num, dropout = dropout)),
        )
                # channel-like attention
        self.B_CA = nn.Sequential(
            Conv_PreNormResidual(channel_num, Channel_Attention(dim = channel_num, heads=channel_num//2, dropout = dropout, window_size = window_size)),
            # Conv_PreNormResidual(channel_num, Gated_Conv_FeedForward(dim = channel_num, dropout = dropout)),
        )

        self.G_SA = nn.Sequential(
            ...
            # Conv_PreNormResidual(channel_num, Gated_Conv_FeedForward(dim = channel_num, dropout = dropout)),
                ...
                # channel-like attention
        self.G_CA = nn.Sequential(
            Conv_PreNormResidual(channel_num, Channel_Attention_grid(dim = channel_num, heads=channel_num//2, dropout = dropout, window_size = window_size)),
            # Conv_PreNormResidual(channel_num, Gated_Conv_FeedForward(dim = channel_num, dropout = dropout)),
        )
